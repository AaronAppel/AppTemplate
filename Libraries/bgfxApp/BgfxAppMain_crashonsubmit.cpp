...
namespace
{
	...
	public:
		...
		void init(int32_t _argc, const char* const* _argv, uint32_t _width, uint32_t _height) override
		{
			...
			m_window = glfwCreateWindow(1280
				, 720
				, "bgfx"
				, NULL
				, NULL
			...
			const bool exists = std::filesystem::exists("shaders/dx11/vs_cubes.bin");
			...
			// if (!entry::processEvents(m_width, m_height, m_debug, m_reset, &m_mouseState))
			{
				...
				for (uint32_t yy = 0; yy < 11; ++yy)
				{
					for (uint32_t xx = 0; xx < 11; ++xx)
					{
						...
						// bgfx::submit(0, m_program); // #BUG Submit is crashing
					}
				}
...
int _main_(int _argc, char** _argv) {
	ExampleCubes _app("01-cubes", "Rendering simple static mesh.", "https://bkaradzic.github.io/bgfx/examples.html#cubes");
